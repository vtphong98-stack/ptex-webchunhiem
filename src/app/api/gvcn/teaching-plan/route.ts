...
import { parseTeachingPlanWorkbook } from "@/lib/excel-teaching-plan";
...
export async function POST(request: Request) {
  ...
  const data = parseTeachingPlanWorkbook(await file.arrayBuffer());
  const total = data.lop11.length + data.lop12.length;
  if (!total) return NextResponse.json({ error: "File không có dữ liệu. Kiểm tra lại sheet Lớp 11 / Lớp 12." }, { status: 400 });
  ...
  await configs.updateOne(
    { schoolYearId },
    { $set: { teachingPlanJson: JSON.stringify(data), teachingPlanUpdatedAt: now, updatedAt: now } },
  );

  revalidatePath("/");
  revalidatePath("/bao-giang");
  return NextResponse.json({ ok: true, updatedAt: now, lop11: data.lop11.length, lop12: data.lop12.length });
}
